...
public class PlayerInventory : MonoBehaviour
{
    ...
    // Finds an empty slot and adds a passive item of a certain type, returns
    // the slot number that the item was put in.
    public int Add(PassiveData data)
    {
        ...
        // Otherwise create the passive item in the slot.
        // Get the type of the passive item we want to spawn.
        GameObject go = new GameObject(data.baseStats.name + " Passive");
        Passive p = go.AddComponent<Passive>();
        p.Initialise(data);
        p.transform.SetParent(transform); // Set the passive item to be a child of the player
        p.transform.localPosition = Vector2.zero;

        // Assign the passive item to the slot
    ...
    // If we don't know whaw item is being added, this function will determine that
    ...
    public void LevelUpWeapon(int slotIndex, int upgradeIndex)
    {
        if (weaponSlots.Count > slotIndex)
        {
            ...
            if (!weapon.DoLevelUp())
            {
                Debug.LogWarning(string.Format("Failed it level up {0}.", weapon.name));
                return;
            }
        }
    ...
    public void LevelUpPassiveItem(int slotIndex, int upgradeIndex)
    {
        if (passiveSlots.Count > slotIndex)
        {
            ...
            if (!p.DoLevelUp())
            {
                Debug.LogWarning(string.Format("Failed it level up {0}.", p.name));
                return;
            }
        }
        ...
        foreach (UpgradeUI upgradeOption in upgradeUIOptions)
        {
            ...
            else if (upgradeType == 2)
            {
                ...
                if (chosenPassiveUpgrade != null)
                {
                    ...
                    // Loops through all our existing passive items. If we find a match, we will
                    // hook an Event Listener to the button that will level up the passive item
                    // when this upgrade option is clicked.
                    bool isLevelUp = false;
                    for (int i = 0; i < passiveSlots.Count; i++)
                    {
                        ...
                        {
                            // If the passive item is already at the max level, do not allow upgrade
                    ...
                    if (!isLevelUp)
                    {
                        ...
                        upgradeOption.upgradeNameDisplay.text = chosenPassiveUpgrade.baseStats.name; // Apply initial name
                        upgradeOption.upgradeDescriptionDisplay.text = chosenPassiveUpgrade.baseStats.description; // Apply initial description
                        upgradeOption.upgradeIcon.sprite = chosenPassiveUpgrade.icon;
                    }
                }
            }
